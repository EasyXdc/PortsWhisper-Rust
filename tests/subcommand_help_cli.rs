...
fn logs_help_uses_clap_managed_help_output() {
    ...
    assert!(stdout.contains("Usage: ports logs"), "unexpected stdout: {stdout}");
    assert!(!stderr.contains("not a valid port/PID"), "unexpected stderr: {stderr}");
}
...
fn kill_help_uses_clap_managed_help_output() {
    ...
    assert!(output.status.success(), "expected success, got: {output:?}");
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Usage: ports kill"), "unexpected stdout: {stdout}");
}
...
fn watch_help_uses_clap_managed_help_output() {
    ...
    assert!(output.status.success(), "expected success, got: {output:?}");
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Usage: ports watch"), "unexpected stdout: {stdout}");
}
...
fn ps_help_uses_clap_managed_help_output() {
    ...
    assert!(output.status.success(), "expected success, got: {output:?}");
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    assert!(stdout.contains("Usage: ports ps"), "unexpected stdout: {stdout}");
}
...
fn completion_help_uses_clap_managed_help_output() {
    ...
    assert!(stdout.contains("Usage: ports completion"), "unexpected stdout: {stdout}");
    assert!(!stderr.contains("unexpected argument '--help'"), "unexpected stderr: {stderr}");
}
...
fn check_help_uses_clap_managed_help_output() {
    ...
    assert!(stdout.contains("Usage: ports check"), "unexpected stdout: {stdout}");
    assert!(!stderr.contains("unexpected argument '--help'"), "unexpected stderr: {stderr}");
}
...
fn open_help_uses_clap_managed_help_output() {
    ...
    assert!(stdout.contains("Usage: ports open"), "unexpected stdout: {stdout}");
    assert!(!stderr.contains("unexpected argument '--help'"), "unexpected stderr: {stderr}");
}
