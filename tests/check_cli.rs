...
fn check_reports_occupied_ports_and_exits_non_zero() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "expected occupied ports to exit 1, got: {output:?}"
    );
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    let stderr = String::from_utf8(output.stderr).expect("stderr should be utf-8");
...
fn check_requires_at_least_one_port_argument() {
    ...
    assert!(
        stderr.contains("Usage: ports check") || stderr.contains("required"),
...
fn check_rejects_non_numeric_port_arguments() {
    ...
    assert!(
        stderr.contains("invalid")
            ...
            || stderr.contains("Usage: ports check"),
        "unexpected stderr: {stderr}"
    );
}
