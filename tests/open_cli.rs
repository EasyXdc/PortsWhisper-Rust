...
fn open_requires_a_port_argument() {
    ...
    assert!(stderr.contains("Usage: ports open") || stderr.contains("required"), "unexpected stderr: {stderr}");
}
...
fn open_rejects_non_numeric_port_arguments() {
    ...
    assert!(stdout.trim().is_empty(), "unexpected stdout: {stdout}");
    assert!(
        stderr.contains("invalid") || stderr.contains("value") || stderr.contains("Usage: ports open"),
...
fn open_rejects_json_output() {
    ...
    assert_eq!(output.status.code(), Some(1), "expected failure, got: {output:?}");
    let stdout = String::from_utf8(output.stdout).expect("stdout should be utf-8");
    let stderr = String::from_utf8(output.stderr).expect("stderr should be utf-8");

    assert!(stdout.trim().is_empty(), "unexpected stdout: {stdout}");
    assert!(stderr.contains("JSON output is not supported for 'open' yet."), "unexpected stderr: {stderr}");
}
